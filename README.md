...
| Skill | 描述 | 入口文件 |
| --- | --- | --- |
| **zan-blockchain-skill** | ZAN 区块链基础设施（API Key 接入） | [SKILL.md](skills/zan-blockchain-skill/SKILL.md) |
| **x402-integration-skill** | x402 协议集成（钱包鉴权 + USDC 即用即付） | [SKILL.md](skills/x402-integration-skill/SKILL.md) |

### 支持的能力范围

**zan-blockchain-skill（API Key 接入）**
...
**x402-integration-skill（x402 即用即付）**

| 分类 | 能力 | 参考文档 |
| --- | --- | --- |
| **SDK 快速上手** | 安装、初始化、首次 RPC 调用 | [sdk-quickstart.md](skills/x402-integration-skill/references/sdk-quickstart.md) |
| **自动支付** | x402 协议 402 付费流程、EIP-3009 / SPL 签名 | [auto-payment.md](skills/x402-integration-skill/references/auto-payment.md) |
| **额度管理** | 余额查询、购买额度、用量分析 | [credits-api.md](skills/x402-integration-skill/references/credits-api.md) |
| **发现 API** | 网关健康、可用网络、套餐、x402 能力声明 | [discovery-api.md](skills/x402-integration-skill/references/discovery-api.md) |
...
| **x402 Payment** | AI Agent、实验探索、即用即付 | 钱包鉴权、USDC 结算、无需注册 |
...
cp -r zan-blockchain-skills/skills/x402-integration-skill ~/.claude/skills/x402-integration-skill
...
将本仓库克隆到项目目录中，Claude Code 会自动发现 `skills/` 下的 SKILL.md 并加载。

### 安装验证

安装后，在 Agent 中尝试以下提问来验证：

**zan-blockchain-skill（API Key）：**
- "帮我查询这个地址的 ETH 余额"
- "查询 BAYC 的 NFT 持有者列表"
- "加速我的 Solana 交易"

**x402-integration-skill（x402 即用即付）：**
- "用 x402 SDK 查询最新的 ETH 区块号"
- "帮我用 USDC 购买 RPC 额度"
- "查看 x402 网关支持哪些链"
...
└── skills/
    ...
    └── x402-integration-skill/                  # x402 即用即付
        ├── SKILL.md                             # Skill 入口（Agent 自动加载）
        └── references/                          # 详细参考（Agent 按需加载）
            ├── sdk-quickstart.md                # SDK 安装与快速上手
            ├── auto-payment.md                  # x402 自动支付流程
            ├── credits-api.md                   # 额度管理 API
            └── discovery-api.md                 # 网关发现 API
```
...
| 用户意图 | 推荐 Skill | 接入方式 |
| --- | --- | --- |
| 余额、交易、区块、Gas、合约调用 | **Chain RPC** | API Key |
| 分析、持仓、NFT、调用链、资金流 | **Data** | API Key |
| Solana 交易加速、Trading Boost | **Trading** | API Key |
| 配额、余量、资源状态 | **Operational** | API Key |
| 无需注册、用钱包调用 RPC | **x402 RPC** | x402 |
| 用 USDC 购买/管理额度 | **x402 Credits** | x402 |
| 探测网关支持的链和套餐 | **x402 Discovery** | x402 |

### 接入方式选择

| 维度 | API Key | x402 |
| --- | --- | --- |
| 注册 | 需要（ZAN Console） | 不需要 |
| 鉴权 | API Key 在 URL 中 | 钱包签名（SIWE/SIWS） |
| 付费 | 订阅 / 预付费 | 按调用付费（USDC） |
| 启动时间 | 分钟级 | 秒级（有钱包即可） |
| 适合场景 | 稳定工作负载 | AI Agent、实验、动态探索 |
...
- 需要返回原始结果还是结构化分析结果？
- 是否需要附带代码示例？
- 使用 API Key 还是 x402 即用即付方式？

## 定价

- **API Key 订阅**：请访问 [ZAN 官网](https://zan.top/pricing) 查看最新套餐与定价
- **x402 即用即付**：通过 SDK 的 `client.listBundles()` 查看实时额度包与 USDC 定价
